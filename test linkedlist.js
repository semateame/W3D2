...
describe("LinkedList", function () {
    ...
    describe("iterator.insert(e) method", function() {
        ...
        describe ("contains", function() {
            let arr = ["big", "small", "tall", "short", "round", "square", "enormous",
            "tiny","gargantuan", "lilliputian", "numberless", "none", "vast",
            "miniscule"]
            arr=arr.sort()
            
            it("finds string number in the array", function () {
                let i = linkedList.contains("number", arr);
                assert.equal(i, -1);
            });    
            it("finds tiny in the array", function () {
                let i = linkedList.contains("tiny", arr);
                assert.equal(i, 12);
            });    
        ...
        describe ("Finds",function (){
            ...
            it("checks if target in the node", function(){
               let i= linkedList.find(node1,3);
               assert.equal(i,false)
            })
            it("checks if target in the node", function(){
                let i= linkedList.find(node3,3);
                assert.equal(i,true)
             })
        })
    });
